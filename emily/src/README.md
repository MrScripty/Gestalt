...
| `model/` | Focused model submodules for episode, outcome, and audit contracts |
...
Provide a modular Emily core that can ingest terminal text, persist text/vector state, record episode-oriented runtime artifacts, and answer context/history queries without coupling Gestalt to one database API or one embedding provider.
...
Use trait-based API/store abstractions with a default runtime wiring focused on persistence, semantic retrieval, episode/outcome capture, vectorization, and runtime diagnostics.
