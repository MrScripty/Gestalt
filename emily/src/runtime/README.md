...
- `sovereign.rs` produces routing decisions, remote episodes, explicit remote state transitions, validation outcomes, deterministic sovereign audit records, and bounded sovereign lifecycle reconciliation through existing crate contracts. Automatic audit generation is intentionally limited to write-side lifecycle events for now.
