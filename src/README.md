...
| `local_agent_membrane.rs` | Host-side local-only membrane execution helper for the local-agent flow |
...
- `local_agent_membrane` keeps local-only membrane execution out of the Dioxus component layer, reuses the existing Emily bridge instead of opening a second runtime, and gates the first real host adoption behind `GESTALT_ENABLE_LOCAL_AGENT_MEMBRANE=1`.
