...
| `emily_local_agent_membrane.rs` | Local-agent local-only membrane adoption coverage through the Emily bridge |
...
- Local-agent membrane tests verify route, validation, and sovereign audit persistence through the bridge-backed membrane path instead of opening a second Emily runtime in the host flow.
