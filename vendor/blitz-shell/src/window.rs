...
use blitz_traits::events::{
    BlitzMouseButtonEvent, BlitzWheelDeltaMode, BlitzWheelEvent, MouseEventButton,
    ...
    pub fn handle_winit_event(&mut self, event: WindowEvent) {
        match event {
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                let (delta_x, delta_y, delta_mode, scroll_x, scroll_y) = match delta {
                ...
                let event_state = self.doc.handle_ui_event(event);

                let has_changed = if event_state.is_cancelled() {
                    false
                } else if let Some(hover_node_id) = self.doc.get_hover_node_id() {
                ...
                if has_changed || event_state.is_cancelled() || event_state.redraw_is_requested() {
                    self.request_redraw();
                }
            }
