...
use blitz_traits::events::{
    BlitzMouseButtonEvent, BlitzWheelEvent, DomEvent, DomEventData, EventState, UiEvent,
};
...
impl<'doc, Handler: EventHandler> EventDriver<'doc, Handler> {
    ...
    pub fn handle_ui_event(&mut self, event: UiEvent) -> EventState {
        ...
        let data = match event {
            ...
            UiEvent::Wheel(data) => DomEventData::Wheel(BlitzWheelEvent {
                x: data.x + viewport_scroll.x as f32 / zoom,
                y: data.y + viewport_scroll.y as f32 / zoom,
                ..data
            }),
    ...
    pub fn handle_dom_event(&mut self, event: DomEvent) -> EventState {
        let mut queue = VecDeque::with_capacity(4);
        queue.push_back(event);
        let mut last_state = EventState::default();
