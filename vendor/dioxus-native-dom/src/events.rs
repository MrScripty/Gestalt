use blitz_traits::events::{
    BlitzKeyEvent, BlitzMouseButtonEvent, BlitzWheelDeltaMode, BlitzWheelEvent, MouseEventButton,
};
use dioxus_html::{
    ...
    MouseData, PlatformEventData, PointerData, ResizeData, ScrollData, SelectionData, ToggleData,
...
impl HasWheelData for NativeWheelData {
    fn delta(&self) -> WheelDelta {
        match self.0.delta_mode {
            BlitzWheelDeltaMode::Pixels => {
                WheelDelta::pixels(self.0.delta_x, self.0.delta_y, self.0.delta_z)
            }
            BlitzWheelDeltaMode::Lines => {
                WheelDelta::lines(self.0.delta_x, self.0.delta_y, self.0.delta_z)
            }
            BlitzWheelDeltaMode::Pages => {
                WheelDelta::pages(self.0.delta_x, self.0.delta_y, self.0.delta_z)
            }
        }
    }
