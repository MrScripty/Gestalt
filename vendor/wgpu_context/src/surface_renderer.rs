...
impl<'s> SurfaceRenderer<'s> {
    ...
    fn acquire_surface_texture(&self) -> Result<SurfaceTexture, SurfaceError> {
    ...
    pub fn maybe_blit_and_present(&self) {
        let surface_texture = match self.acquire_surface_texture() {
            ...
            Err(SurfaceError::Timeout | SurfaceError::Outdated | SurfaceError::Lost) => return,
            Err(SurfaceError::Other) => return,
            Err(SurfaceError::OutOfMemory) => panic!("failed to get surface texture: out of memory"),
        };
